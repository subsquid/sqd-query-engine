...
| [INV-Q14](07-invariants.md#inv-q14) | CT-2 | **P** | `the_field_surface_is_exactly_the_declared_one` compares every table's declared list, as a set, and then parses each name — but against `REFERENCE_FIELD_SURFACE`, a hand transcription of the reference taken on the day someone typed it. The reference moving is exactly the break the invariant is about, and the transcription cannot see it: it stayed green through the thirteen fields of gap 35. `a_column_the_catalog_carries_is_not_a_field` covers `blockNumber` and `topic0` |
