...
| [INV-P16](07-invariants.md#inv-p16) | CT-3 | **P** | the pruning-disabled equality run exists: `a_filter_returns_the_same_rows_with_nothing_to_prune_on` runs five filters — matching, non-matching, conjunctive, empty-list, absent — against chunks written with no statistics to prune on, a row per row group, a row per data page, and no dictionary, and compares bytes. `a_nested_column_earlier_in_the_schema_does_not_reach_the_answer` adds the case that made the invariant load-bearing: parquet counts a file's columns in leaves and Arrow counts them in fields, so a `List<Struct<…>>` earlier in the schema put every filter after it on another column's bounds. Early termination is CT-5's `scan` module. Filter reordering is not varied, and it is five filters over one query shape rather than every fixture query |
