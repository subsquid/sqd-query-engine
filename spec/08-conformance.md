...
| Drop a relation's target when no row points at it | Unchanged output ([INV-E4](07-invariants.md#inv-e4)) |
| Add a stray or unreadable file nothing names | Unchanged output ([INV-E4](07-invariants.md#inv-e4)) |
...
Dated **2026-09-12**. Every invariant, the class that checks it, and an honest
status:
...
| [INV-D6](07-invariants.md#inv-d6) | CT-1 | **P** | one case inside `test_validate_rejects_a_relation_that_cannot_join` — a `children` relation whose *target* table has no address column. It asserts `is_err()` without the message, so it would pass on any of the other checks the scaffolding trips, and the three remaining halves of the invariant have no case at all: a *source* table without one, and `parents` in either position |
| [INV-D7](07-invariants.md#inv-d7) | CT-6, CT-8 | **C** | `physical_width_does_not_reach_the_answer` rewrites one chunk at every one of the eight integer widths, for every integer column that can hold its values, and byte-compares each; `narrowing_every_column_at_once_does_not_reach_the_answer` the combination one column at a time hides; `a_narrow_column_in_a_shuffled_chunk_does_not_reach_the_answer` the pairing with a storage order that is not the item-key order, which is what reaches the sort comparator at all. `a_list_key_answers_the_same_at_any_element_width` covers the element types inside a list on a real chunk, and `a_fixture_chunk_answers_the_same_at_any_width` the scalar widths there. The filtered columns, which the sweep above reaches only through a string filter: `an_integer_filter_reads_every_physical_width` retypes a one-byte discriminator and an item index to every width under a plain list, the discriminator filter and an index list, and `a_boolean_filter_reads_a_flag_stored_as_a_number` a flag stored as a number. Underneath: `a_narrower_column_orders_against_a_wider_one_by_value`, block-range masks, in-lists (`an_unsigned_list_reads_every_physical_width`, `a_value_the_stored_width_cannot_hold_matches_nothing`, `a_string_list_reads_every_text_type`), scalars (`a_scalar_reads_every_physical_width`, `a_boolean_reads_a_numeric_column`), semi-joins, range predicates (`a_range_reads_every_physical_width`), `test_solana_tx_version_reads_the_sentinel_at_every_physical_width`, `test_bignum_reads_a_narrowed_column` |
...
| [INV-Q14](07-invariants.md#inv-q14) | CT-2 | **C** | `the_field_surface_is_exactly_the_declared_one` compares every table's declared list against a transcription of the reference's, as a set, and then parses each name. The transcription alone stayed green through the thirteen fields of gap 35, so `the_catalogs_serve_every_field_the_reference_serves` recomputes it from the reference's own `item_field_selection!` blocks wherever the fixture tree's checkout is present, both ways: a field the reference adds fails here before a review finds it. `a_column_the_catalog_carries_is_not_a_field` covers `blockNumber` and `topic0` |
...
| [INV-P7](07-invariants.md#inv-p7) | CT-3 | **P** | the disjunction cases cover null propagation, and `a_null_is_not_greater_than_the_constant` the one filter kind whose comparison hands the scan a mask with nulls in it, and `a_null_row_stays_null_under_an_out_of_width_threshold` the shortcut a threshold outside the stored width takes; still no per-filter-kind sweep |
...
| [INV-P13](07-invariants.md#inv-p13) | CT-3 | **P** | nothing sends a mixed-length discriminator and checks the rows that come back, which is the invariant's own test. `test_compile_discriminator_mixed_lengths` asserts the *shape* the compiler builds — two predicates, two columns each — so a disjunction evaluated as a conjunction would still pass it. `discriminator_hex_is_a_prefix_chain` filters on `programId` alone and checks a property of the returned data, not of dispatch. The Kleene cases in the predicate unit tests cover the null propagation. `"0x"` matching every row has no case |
...
| [INV-P16](07-invariants.md#inv-p16) | CT-3 | **P** | `pruning_never_drops_a_row_the_mask_would_keep` states the decision as a law — a group may be pruned only where the mask would have emptied it — over the eight widths a writer may pick, each column holding its own extremes, against every filter kind the plan compiles on an integer column (equality, in-list, both ranges) at every value kind it compiles them with (`uint8` through `uint64`, `int16`, `int64`), across each width's boundaries. Each of the fix's three arms fails it under mutation. It caught pruning that read a statistic at the parquet physical type while evaluation narrowed to the stored one, so a `uint8` 255 pruned the group holding the `-1` an `Int8` column stores it as, and the answer turned on whether the writer had recorded statistics. `a_statistic_that_is_not_text_prunes_nothing` covers the decline half the invariant names. The equality run is `a_filter_returns_the_same_rows_with_nothing_to_prune_on`: five filters — matching, non-matching, conjunctive, empty-list, absent — against chunks written with no statistics, a row per row group, a row per data page, and no dictionary, compared as bytes; `a_nested_column_earlier_in_the_schema_does_not_reach_the_answer` adds the case that made the invariant load-bearing, a `List<Struct<…>>` earlier in the schema putting every filter after it on another column's bounds. What a plausible break still passes: the law is stated over integer columns only, so text pruning rests on the byte-equality run alone; filter reordering is not varied; early termination is CT-5's `scan` module; and the equality run is five filters over one query shape rather than the every-fixture-query sweep the invariant asks for |
...
| [INV-O10](07-invariants.md#inv-o10) | CT-6 | **C** | `test_encode_roll`, `test_encode_roll_with_list_spread`; `a_roll_renders_its_sources_at_their_declared_encoding` that a source renders in the roll as it would alone |
...
| [INV-E4](07-invariants.md#inv-e4) | CT-8 | **C** | `a_missing_table_is_an_error`, `a_missing_relation_table_is_an_error`, and `a_missing_block_table_is_an_error` cover every way a plan names a table; `a_missing_relation_table_is_not_observed_without_inputs` and `only_the_tables_a_query_names_are_opened` cover when it is not asked for |
...
| [INV-E7](07-invariants.md#inv-e7) | CT-4, CT-8 | **C** | `test_semi_join_unsupported_key_type` for a join key. For a filter column, `a_filter_on_an_uncomparable_column_is_an_error` retypes a discriminator, a flag, an index and a program id under every filter kind that reaches a column — a program id stored as bytes among them, which read as UTF-8 would answer "no rows" — and asserts the kind; `an_uncomparable_column_is_refused_by_every_predicate_kind` and `test_list_contains_any_on_a_non_list_is_refused` the predicates underneath, and `the_stored_type_check_agrees_with_evaluation` that the pre-read probe and the evaluation are one answer. The other direction — a type that is comparable and used to panic — is `a_bloom_and_a_dictionary_of_any_key_width_are_read`, with `a_bloom_reads_every_bytes_type` and `a_dictionary_keeps_its_null_keys_out_of_the_answer` underneath |
