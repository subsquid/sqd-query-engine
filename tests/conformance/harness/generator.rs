...
use crate::harness::fixtures::{run_against, run_against_with_cut};
...
impl Generator {
    ...
    pub fn run_with(&self, query: &str, options: ExecOptions) -> Vec<u8> {
        self.run_with_cut(query, options).0
    }

    /// The same, retaining the last completely read block on an early stop.
    pub fn run_with_cut(&self, query: &str, options: ExecOptions) -> (Vec<u8>, Option<u64>) {
        run_against_with_cut(&self.catalog, &self.chunk, query, options)
            .unwrap_or_else(|e| panic!("a generated query failed: {e:#}\n  query: {query}"))
    }
}
