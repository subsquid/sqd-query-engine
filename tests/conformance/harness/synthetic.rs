...
// A block-partitioned log table with heavy blocks shared by adjacent row groups.
// Transactions cover the same blocks in one group, exercising mixed layouts.
...
pub fn partitioned_chunk(log_weight: u64) -> TempDir {
    ...
    // group, exercising range reads across overlapping layouts.
    write_table(
        dir.path(),
        "transactions",
        vec![
...
/// Page the chunk in a pool of the given size.
pub fn paged_at(
    meta: &DatasetDescription,
    chunk: &TempDir,
    to: u64,
